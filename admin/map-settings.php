...
settings_errors();?>
    <div id="general">
        ...
        <div id="tabs-1">
            <form id="dealer_map-settings-form" method="post" action="options.php" autocomplete="off"
                ...
                <div class="metabox-holder">
                    <div id="dealer_map-api-settings" class="postbox">
                        ...
                        <div class="inside">
                            <p>
                                <label
                                    for="dealer_map-api-server-key"><?php esc_html_e('Google Map API key', 'wp-dealer-map');?>:</label>
                                <input type="text" value="<?php echo esc_attr($dealer_map_api_server); ?>"
                                    name="dealer_map_api[server_key]" class="textinput" size="40"
                                    id="dealer_map-api-server-key">
                            </p>
                            <p>
                            ...
                            <p>
                                <label
                                    for="dealer_map-api-language"><?php esc_html_e('Map language', 'wp-dealer-map');?>:</label>
                            ...
                            <p>
                                <label
                                    for="dealer_map-api-region"><?php esc_html_e('Map region', 'wp-dealer-map');?>:</label>
                                <select id="dealer_map-api-region" name="dealer_map_api[region]">
                                    <?php echo $dealer_map_admin->settings_page->get_api_reg_list(); ?>
                                </select>
                            </p>
                            <p class="submit">
                                <input type="submit" value="<?php esc_attr_e('Save Changes', 'wp-dealer-map');?>"
                ...
                <div id="dealer_map-search-settings" class="postbox">
                    ...
                    <div class="inside">
                        <div class="padd">
                            ...
                            <p>
                                <label
                                    for="dealer_map-def-latitude"><?php esc_html_e('Default Starting Latitude', 'wp-dealer-map');?>::</label>
                                <input type="number" step="any"
                                    placeholder="<?php echo esc_attr($dealer_map_def_lat); ?>"
                                    name="dealer_map_search[def_lat]" class="textinput" size="25"
                                    id="dealer_map-def-latitude">
                            </p>
                            <p>
                                <label
                                    for="dealer_map-def-longitude"><?php esc_html_e('Default Starting Longitude', 'wp-dealer-map');?>::</label>
                                <input type="number" step="any"
                                    placeholder="<?php echo esc_attr($dealer_map_def_lng); ?>"
                                    name="dealer_map_search[def_lng]" class="textinput" size="25"
                                    id="dealer_map-def-longitude">
                            </p>
                            <p>
                                <label
                                    for="dealer_map-def-range"><?php esc_html_e('Default range', 'wp-dealer-map');?>::</label>
                                <input type="number" step="5" value="<?php echo esc_attr($dealer_map_def_range); ?>"
                                    name="dealer_map_search[def_range]" class="textinput" size="10"
                                    id="dealer_map-def-range">
                            </p>
                            <p>
                                <label
                                    for="dealer_map-def-limit"><?php esc_html_e('Default limit', 'wp-dealer-map');?>::</label>
                                <input type="number" step="2" value="<?php echo esc_attr($dealer_map_def_limit); ?>"
                                    name="dealer_map_search[def_limit]" class="textinput" size="10"
                                    id="dealer_map-def-limit">
                            </p>
                        </div>
                        <p>
                            <label
                                for="dealer_map-show-default"><?php esc_html_e('Show default dealers on map?', 'wp-dealer-map');?></label>
                            <input type="checkbox" value="" <?php checked($dealer_map_show_default, true);?>
                                name="dealer_map_search[show_def]" id="dealer_map-show-default">
                        </p>
                        <p>
                            <label
                                for="dealer_map-default-address"><?php esc_html_e('Default ZIP Starting Point', 'wp-dealer-map');?>::</label>
                            <input type="number" value="<?php echo esc_attr($dealer_map_d_address); ?>"
                                name="dealer_map_search[defaddress]" class="textinput" size="25"
                                id="dealer_map-default-address">
                        </p>
                        <p>
                            <label
                                for="dealer_map-results-dropdown"><?php esc_html_e('Show the max results dropdown?', 'wp-dealer-map');?></label>
                            <input type="checkbox" value="" <?php checked($dealer_map_results_drop, true);?>
                                name="dealer_map_search[results_dropdown]" id="dealer_map-results-dropdown">
                        </p>
                        <p>
                            <label
                                for="dealer_map-radius-dropdown"><?php esc_html_e('Show the search radius dropdown?', 'wp-dealer-map');?></label>
                            <input type="checkbox" value="" <?php checked($dealer_map_radius_drop, true);?>
                                name="dealer_map_search[radius_dropdown]" id="dealer_map-radius-dropdown">
                        </p>
                        <p>
                            <label
                                for="dealer_map-distance-unit"><?php esc_html_e('Distance unit', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="km"
                                    <?php checked('km', $dealer_map_distance_unit);?>
                                    name="dealer_map_search[distance_unit]" id="dealer_map-distance-km">
                                <label for="dealer_map-distance-km"><?php esc_html_e('km', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="mi"
                                    <?php checked('mi', $dealer_map_distance_unit);?>
                                    name="dealer_map_search[distance_unit]" id="dealer_map-distance-mi">
                                <label for="dealer_map-distance-mi"><?php esc_html_e('mi', 'wp-dealer-map');?></label>
                            </span>
                        </p>
                        <p>
                            <label
                                for="dealer_map-max-results"><?php esc_html_e('Max search results', 'wp-dealer-map');?>:</label>
                            <input type="text" value="<?php echo esc_attr($dealer_map_max_results); ?>"
                                name="dealer_map_search[max_results]" class="textinput" size="40"
                                id="dealer_map-max-results">
                        </p>
                        <p>
                            <label
                                for="dealer_map-search-radius"><?php esc_html_e('Search radius options', 'wp-dealer-map');?>:</label>
                            <input type="text" value="<?php echo esc_attr($dealer_map_max_radius); ?>"
                                name="dealer_map_search[radius]" class="textinput" size="40"
                                id="dealer_map-search-radius">
                        </p>
                        <p class="submit">
                            <input type="submit" value="<?php esc_html_e('Save Changes', 'wp-dealer-map');?>"
                                class="button-primary">
                        </p>
                    </div>
                </div>
            ...
            <div class="metabox-holder">
                <div id="dealer_map-layout-settings" class="postbox">
                    ...
                    <div class="inside">
                        <p>
                            <label
                                for="dealer_map-map-height"><?php esc_html_e('Map height (in pixels)', 'wp-dealer-map');?>:</label>
                            <input type="number" step="10" max="2000"
                                value="<?php echo esc_attr($dealer_map_map_height); ?>"
                                name="dealer_map_layout[map_height]" class="textinput" size="10"
                                id="dealer_map-map-height">
                        </p>
                        <p>
                            <label
                                for="dealer_map-layout-type"><?php esc_html_e('Select map type', 'wp-dealer-map');?>:</label>
                        ...
                        <p>
                            <label
                                for="dealer_map-layout-zoom"><?php esc_html_e('Select zoom level', 'wp-dealer-map');?>:</label>
                        ...
                        <p>
                            <label
                                for="dealer_map-marker-event"><?php esc_html_e('Show infobox on', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="click"
                                    <?php checked('click', $dealer_map_marker_event);?>
                                    name="dealer_map_layout[marker_event]" id="dealer_map-marker-click">
                                <label
                                    for="dealer_map-marker-click"><?php esc_html_e('Click', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="mouseover"
                                    <?php checked('mouseover', $dealer_map_marker_event);?>
                                    name="dealer_map_layout[marker_event]" id="dealer_map-marker-mouseover">
                                <label
                                    for="dealer_map-marker-mouseover"><?php esc_html_e('Mouseover', 'wp-dealer-map');?></label>
                            </span>
                        </p>
                        <p>
                            <label
                                for="dealer_map-marker-effect"><?php esc_html_e('Marker effect on', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="drop"
                                    <?php checked('drop', $dealer_map_marker_effect);?>
                                    name="dealer_map_layout[marker_effect]" id="dealer_map-marker-drop">
                                <label for="dealer_map-marker-drop"><?php esc_html_e('Drop', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="bounce"
                                    <?php checked('bounce', $dealer_map_marker_effect);?>
                                    name="dealer_map_layout[marker_effect]" id="dealer_map-marker-bounce">
                                <label
                                    for="dealer_map-marker-bounce"><?php esc_html_e('Bounce', 'wp-dealer-map');?></label>
                            </span>
                        </p>
                        <p>
                            <label
                                for="dealer_map-address_columns"><?php esc_html_e('Number of result columns', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="2"
                                    <?php checked('2', $dealer_map_num_columns);?>
                                    name="dealer_map_layout[num_of_columns]" id="dealer_map-marker-two">
                                <label for="dealer_map-marker-two"><?php esc_html_e('Two', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="3"
                                    <?php checked('3', $dealer_map_num_columns);?>
                                    name="dealer_map_layout[num_of_columns]" id="dealer_map-marker-three">
                                <label
                                    for="dealer_map-marker-three"><?php esc_html_e('Three', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="4"
                                    <?php checked('4', $dealer_map_num_columns);?>
                                    name="dealer_map_layout[num_of_columns]" id="dealer_map-marker-four">
                                <label for="dealer_map-marker-four"><?php esc_html_e('Four', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="5"
                                    <?php checked('5', $dealer_map_num_columns);?>
                                    name="dealer_map_layout[num_of_columns]" id="dealer_map-marker-five">
                                <label for="dealer_map-marker-five"><?php esc_html_e('Five', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="6"
                                    <?php checked('6', $dealer_map_num_columns);?>
                                    name="dealer_map_layout[num_of_columns]" id="dealer_map-marker-six">
                                <label for="dealer_map-marker-six"><?php esc_html_e('Six', 'wp-dealer-map');?></label>
                            </span>
                        </p>
                        <p>
                            <label
                                for="dealer_map-button-css"><?php esc_html_e('Search button styling', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="--"
                                    <?php checked('--', $dealer_map_button_class);?> name="dealer_map_layout[btn_class]"
                                    id="dealer_map-btn-def">
                                <label for="dealer_map-btn-def"><?php esc_html_e('Default', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="blue"
                                    <?php checked('blue', $dealer_map_button_class);?>
                                    name="dealer_map_layout[btn_class]" id="dealer_map-btn-blue">
                                <label for="dealer_map-btn-blue"><?php esc_html_e('Blue', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="red"
                                    <?php checked('red', $dealer_map_button_class);?>
                                    name="dealer_map_layout[btn_class]" id="dealer_map-btn-red">
                                <label for="dealer_map-btn-red"><?php esc_html_e('Red', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="green"
                                    <?php checked('green', $dealer_map_button_class);?>
                                    name="dealer_map_layout[btn_class]" id="dealer_map-btn-green">
                                <label for="dealer_map-btn-green"><?php esc_html_e('Green', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="white"
                                    <?php checked('white', $dealer_map_button_class);?>
                                    name="dealer_map_layout[btn_class]" id="dealer_map-btn-white">
                                <label for="dealer_map-btn-white"><?php esc_html_e('White', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="black"
                                    <?php checked('black', $dealer_map_button_class);?>
                                    name="dealer_map_layout[btn_class]" id="dealer_map-btn-black">
                ...
                <div id="dealer_map-addition-settings" class="postbox">
                    ...
                    <div class="inside">
                        <p>
                            ...
                            <br />
                            <label
                                for="dealer_map-delete-all"><?php esc_html_e('Delete all data', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="remove"
                                    <?php checked('remove', $dealer_map_delete_all);?>
                                    name="dealer_map_addition[remove_all]" id="dealer_map-delete-all">
                                <label
                                    for="dealer_map-delete-all"><?php esc_html_e('Remove', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="keep"
                                    <?php checked('keep', $dealer_map_delete_all);?>
                                    name="dealer_map_addition[remove_all]" id="dealer_map-keep">
                                <label for="dealer_map-keep"><?php esc_html_e('Keep', 'wp-dealer-map');?></label>
                            </span>
                        </p>
                        <p>
                            <label
                                for="dealer_map-keep-table"><?php esc_html_e('Keep table', 'wp-dealer-map');?>:</label>
                            <span class="dealer_map-radioboxes">
                                <input type="radio" autocomplete="off" value="remove"
                                    <?php checked('remove', $dealer_map_keep_table);?>
                                    name="dealer_map_addition[keep_table]" id="dealer_map-keep-remove">
                                <label
                                    for="dealer_map-keep-remove"><?php esc_html_e('Remove', 'wp-dealer-map');?></label>
                                <input type="radio" autocomplete="off" value="keep"
                                    <?php checked('keep', $dealer_map_keep_table);?>
                                    name="dealer_map_addition[keep_table]" id="dealer_map-keep-keep">
                                <label for="dealer_map-keep-keep"><?php esc_html_e('Keep', 'wp-dealer-map');?></label>
                            </span>
                        </p>
                        <div class="padd">
                            <h4><?php esc_html_e('Choose between producting & developing stage', 'wp-dealer-map');?>
                            </h4>
                            <p>
                                <label
                                    for="dealer_map-environment"><?php esc_html_e('Environment', 'wp-dealer-map');?>:</label>
                                <span class="dealer_map-radioboxes">
                                    <input type="radio" autocomplete="off" value="production"
                                        <?php checked('production', $dealer_map_environment);?>
                                        name="dealer_map_addition[environment]" id="dealer_map-en-production">
                                    <label
                                        for="dealer_map-en-production"><?php esc_html_e('Production', 'wp-dealer-map');?></label>
                                    <input type="radio" autocomplete="off" value="developing"
                                        <?php checked('developing', $dealer_map_environment);?>
                                        name="dealer_map_addition[environment]" id="dealer_map-en-developing">
                                    <label
                                        for="dealer_map-en-developing"><?php esc_html_e('Developing', 'wp-dealer-map');?></label>
                                </span>
                            </p>
                            <h5><?php esc_html_e('Deregister other G-maps scripts if necceasary, only if they break things', 'wp-dealer-map');?>
                            </h5>
                            <p>
                                <label
                                    for="dealer_map-dereg_scripts"><?php esc_html_e('Deregister other maps?', 'wp-dealer-map');?></label>
                                <input type="checkbox" value="" <?php checked($dealer_map_oth_script, true);?>
                                    name="dealer_map_addition[gmap_scripts]" id="dealer_map-dereg_scripts">
                            </p>
                        </div>
                        <p class="submit">
                            <input type="submit" value="<?php esc_html_e('Save Changes', 'wp-dealer-map');?>"
        ...
        <div id="tabs-5">
            <form id="dealer_map_form" method="post" action="">
                <div class="metabox-holder">
                    <div class="postbox">
                        <h3 class="hndle">
                            <span><?php esc_html_e('Export/Import Stores to Database', 'wp-dealer-map');?></span>
                        </h3>
                        <div class="inside">
                            <table class="form-table">
                                <tr valign="top">
                                    <th scope="row"><?php esc_html_e('Select Input File:', 'wp-dealer-map');?></th>
                                    <td>
                                        <?php $repop_file = isset($_POST['csv_file']) ? sanitize_text_field($_POST['csv_file']) : null;?>
                                        <input id="csv_file" name="csv_file" type="text" size="70"
                                            value="<?php echo $repop_file; ?>" />
                                        <input id="csv_file_button" type="button" class="button-primary"
                                            value="Upload" />
                                ...
                                <tr valign="top">
                                    <th scope="row"><?php esc_html_e('Update Database Rows:', 'wp-dealer-map');?></th>
                                    <td>
...
if (isset($_POST['execute_button'])) {
    ...
    if ($myCSV && ($fh = @fopen($myCSV, 'r')) !== false) {
        ...
        while (($row = fgetcsv($fh, 1000, ";")) !== false) { // Get file contents and set up row array
...
$dissm_btn = __('Click to dismiss', 'wp-dealer-map');
if (!empty($message_info_style)) {
    echo '<div class="info_message_dismiss">';
    echo $message_info_style;
    echo '<br /><button class="button">' . $dissm_btn . '</button>';
    echo '</div>';
}
...
if (!empty($success_message)) {
    ...
    echo '</div>';
                            ...
                            <p class="submit">
                                <input id="execute_button" name="execute_button" type="submit" class="button-primary"
                                    value="<?php esc_attr_e('Import to DB', 'wp-dealer-map')?>" />
                                <input id="export_to_csv_button" name="export_to_csv_button" type="submit"
                                    class="button-secondary"
                                    value="<?php esc_attr_e('Export to CSV', 'wp-dealer-map')?>" />
                                <input id="delete_db_button" name="delete_db_button" type="button"
                                    class="button-secondary"
                                    value="<?php esc_attr_e('Delete Table', 'wp-dealer-map')?>" />
                                <input type="hidden" id="delete_db_button_hidden" name="delete_db_button_hidden"
                                    value="" />
                            </p>
            </form>
