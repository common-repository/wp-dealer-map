    ...
    class Dealer_Map_Admin
    {
        ...
        public function create_admin_menu()
        {
            $sub_menu = apply_filters('dealer_map_sub_menu_items', array(
                ...
                'icon' => DEALER_URL . 'admin/img/map-pini.png',
            )
            );
...
<div class="wrap">
    ...
    <h2><?php _e('Dealer Locations', 'wp-dealer-map')?>
        <a class="add-new-h2"
            href="<?php echo esc_url(get_admin_url(get_current_blog_id(), 'admin.php?page=dealer_map/locations_form')); ?>">
...
<?php
        ...
        public function dealer_map_validate_data($item)
        {
            $messages = array();
            if (empty($item['name'])) {
                $messages[] = __('Name is required', 'wp-dealer-map');
            }
            if (empty($item['address'])) {
                $messages[] = __('Address is in wrong format', 'wp-dealer-map');
            }
            if (is_int($item['lat'])) {
                $messages[] = __('Latitude is in wrong format', 'wp-dealer-map');
            }
            if (is_int($item['lng'])) {
                $messages[] = __('Longitude is in wrong format', 'wp-dealer-map');
            }
            if (!ctype_digit($item['active'])) {
                $messages[] = __('Active only can be 1 or 0', 'wp-dealer-map');
            }
            if (!ctype_digit($item['proseries'])) {
                $messages[] = __('Pro Series only can be 1 or 0', 'wp-dealer-map');
            }
            if (empty($messages)) {
                return true;
            }
        ...
        public function csv_export($getTable)
        {
            ...
            $getField = '';
            if ($getTable) {
                $result = $wpdb->get_results("SELECT * FROM $getTable ORDER BY id ASC", ARRAY_A);
                $csv_file_name = 'DEALER_STORES_' . date('mdy_His') . '.csv';
                if ($result === null) {
                    //no results, empty
                    wp_die("Can't get database data.");
                }
        ...
        {
            if (strpos($file, 'wp-dealer-map.php') !== false) {
                $settings_link = '<a href="' . admin_url('admin.php?page=wp-dealer-map/admin/class-admin.php') . '" title="' . esc_attr__('View Dealers Store Locator Settings', 'wp-dealer-map') . '">'
